...
--and gives the size of the whole graph
...
data SearchResult v = NoSuccess Int | Success [v] deriving (Eq, Ord, Show, Generic)
{-
...
dfs depthLimit nodeLimit getAdjacent isSuccess startVertex = munge $ loop True Empty [] [(0, startVertex)] where 
  munge :: (Bool, Int, Either Text [v]) -> Either Text (SearchResult v)
  munge = \case 
    (_, _, Right path) -> Right (Success path)
    (True, numNodes, Left _reason) -> Right $ NoSuccess numNodes
    (False, _, Left reason) -> Left $ "search was not exhaustive and " <> reason 
  ...
  loop :: Bool -> Set v -> [v] -> [(Int, v)] -> (Bool, Int, Either Text [v])
  -- loop searchWasExhaustive explored curPath stack | 
  --    trace ("curStack: " <> show stack) False = undefined 
  loop searchWasExhaustive explored curPath stack = if length explored > nodeLimit 
    then (False, length explored, Left "hit nodelimit") else 
    case stack of 
      [] -> (searchWasExhaustive, length explored, Left "ran out of things to search")
      (vDepth, v) : vs -> 
        -- trace ("exploring at depth: " <> showP vDepth <> " " <> showP v
        -- <> "\nsuccess: " <> show (isSuccess v)) $
       let newPath = takeExact vDepth curPath ++ [v] in 
        if isSuccess v 
          then (searchWasExhaustive, length explored, Right newPath)
          else let 
