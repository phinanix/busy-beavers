...
instance Applicative (MultiResult s) where
  pure = return
  x1 <*> x2 = x1 >>= flip fmap x2

instance Monad (MultiResult s) where
  return = NewState
  (>>=) :: MultiResult s a -> (a -> MultiResult s b) -> MultiResult s b
  k >>= f = case k of
    NewState a -> f a
...
liftOneToMulti :: SimOneAction s -> SimMultiAction s
...
simulateStepOneMachine handleUnknown limit machine
 state@(SimState ph tape book steps skipTrace hist histSet counter curDisp dispHist rsHist fastLRCheck)
  = if counter > limit
...
simulateStepPartial limit machine (SimState ph tape book steps skipTrace hist histSet counter curDisp dispHist rsHist fastLRCheck) =
...
recordDispHist :: SimState s -> SimState s
...
{-algorithm: keep the config from a particular big step k around + a readshift since then 
...
eotProof :: SimState Bit -> Maybe (HaltProof Bit)
...
loopForEndOfTapeGlue :: Int -> Turing -> OneLoopRes Bit
