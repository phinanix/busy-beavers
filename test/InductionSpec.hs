...
spec = do
  ...
  describe "proveInductively" $ do
    it "proves a simple thing" $
      proveInductively 20 simple_sweeper (initChainBookBit simple_sweeper) simple_sweeperGoal (BoundVar 0)
      `shouldBe` Right (Induction (initChainBookBit simple_sweeper) 20)
    xit "proves a counter machine counts" $
      proveInductively 20 weird3 (initChainBookBit weird3) weird3Goal (BoundVar 0)
      `shouldBe` Right (Induction (initChainBookBit weird3) 20)
    it "fails to pvoe a thing that is false" $
      proveInductively 20 checkerboardSweeper (initChainBookBit checkerboardSweeper) checkerboardFalseGoal
    ...
    xit "proves a guess for the counter machine" $
      let goal = fromJust $ either (const Nothing) Just $ makeIndGuess 100 weird3 in
        proveInductively 20 weird3 (initChainBookBit weird3) goal (BoundVar 0) `shouldBe` Left ("", Nothing)
